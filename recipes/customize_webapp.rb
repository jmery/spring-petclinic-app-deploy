...
ruby_block 'Wait for CSS file to be writeable' do
  block do
    true until ::File.exist?("#{node['tomcat']['base_dir']}/webapps/#{node['tomcat']['base_instance']}/resources/css/petclinic.css")
  end
end
...
ruby_block 'Wait for Messages file to be writeable' do
  block do
    true until ::File.exist?("#{node['tomcat']['base_dir']}/webapps/#{node['tomcat']['base_instance']}/WEB-INF/classes/messages/messages.properties")
  end
end
